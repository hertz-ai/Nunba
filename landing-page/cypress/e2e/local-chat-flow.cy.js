...
describe('Local Chat Flow', () => {
  ...
  describe('Page Load & Hero Transition', () => {
    it('page loads without black screen', () => {
      ...
      cy.get('#hero-section', { timeout: 300000 }).should(($el) => {
    ...
    it('demo section becomes interactive', () => {
      ...
      cy.get('textarea, input[type="text"]', { timeout: 300000 }).should('exist');
    });
  });
  ...
  describe('Chat with Default Agent', () => {
    it('sending "hi" returns a response without autogen dump', () => {
      // Type and send
      cy.get('textarea, input[type="text"]', { timeout: 300000 }).first().type('hi', { force: true });
    ...
    it('response does not contain raw think tags', () => {
      cy.get('textarea, input[type="text"]', { timeout: 300000 }).first().type('hello{enter}', { force: true });
      cy.wait(15000);
      cy.get('body').then(($body) => {
        const text = $body.text();
  ...
  describe('Toolbar on Mobile', () => {
    ...
    it('toolbar is visible and does not cover full width', () => {
      ...
      cy.get('[class*="z-50"]', { timeout: 300000 }).should('exist');
      // Chat input should be reachable below toolbar
      cy.get('textarea, input[type="text"]', { timeout: 300000 }).first().should('be.visible');
    });
  });

  describe('User Chat Responsiveness', () => {
    it('user chat gets a response (not blocked indefinitely)', () => {
      cy.get('textarea, input[type="text"]', { timeout: 300000 }).first().type('what is 2+2{enter}', { force: true });
