...
export default function Plan() {
  ...
  var userData = location.state && location.state.userData;
  var student = (userData && userData.num_of_students) ?? '';

  const signup = (event) => {
    logger.log('you are here');
  ...
  const makePayment = async (subscription) => {
    ...
    mailerApi
      ...
      .then((data) => {
        const redirect = data;
        logger.log('this is the redirect url', redirect);
  ...
  return (
    <React.Fragment>
      <AppContext.Provider value={{hevolvedroid}}>
        ...
        {!hevolvedroid && <HeaderNano fixed={true} />}
        {/* Hero unit */}
                  ...
                  <CardActions style={{marginTop: 'auto'}}>
                    {subscription.price === 0 ? (
                      <ColorButton
                        fullWidth
                        onClick={(event) =>
                          userData ? handleClick(subscription) : signup(event)
                        }
                      >
                        Register For Free
                      </ColorButton>
