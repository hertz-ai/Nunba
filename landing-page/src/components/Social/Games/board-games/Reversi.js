...
function getFlips(board, r, c, playerID) {
  ...
  for (const [dr, dc] of DIRECTIONS) {
    ...
    while (cr >= 0 && cr < SIZE && cc >= 0 && cc < SIZE && board[cr][cc] === opponentID) {
      flips.push([cr, cc]);
      cr += dr;
      cc += dc;
    }
    ...
    if (flips.length > 0 && cr >= 0 && cr < SIZE && cc >= 0 && cc < SIZE && board[cr][cc] === playerID) {
      allFlips.push(...flips);
    }
  }
                ...
                {cell !== null ? (
                  <Box
                    sx={{
                      ...
                      border: cell === '1' ? '1px solid rgba(0,0,0,0.1)' : 'none',
