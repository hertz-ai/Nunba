...
class TestTheCaptionServerIsNeverAdoptedAsMain:
    ...
    Every test stubs the network: check_server_type decides which ports look
    ...
    def _alive_on(ports):
        ...
        def check(port):
            if port in ports:
                return (ServerType.EXTERNAL_LLAMA, {'status': 'ok'})
            return (ServerType.NOT_RUNNING, None)
        return check

    def _start(self, tmp_config_dir, alive, server_port=None):
        from llama.llama_config import LlamaConfig
        cfg = LlamaConfig(config_dir=tmp_config_dir)
        cfg.config['server_port'] = server_port or self._UNUSED_PORT
        with patch.object(cfg, 'check_server_type', side_effect=self._alive_on(alive)), \
             patch('requests.get', side_effect=OSError('no network in this test')), \
             patch.object(LlamaConfig, '_propagate_llm_url') as propagate:
    ...
    def test_the_excluded_port_follows_the_caption_port_setting(self, tmp_config_dir, monkeypatch):
    ...
    def test_another_apps_server_is_still_reused(self, tmp_config_dir, monkeypatch):
        """The scan's original purpose: reuse a llama-server another app
        (trueflow) already runs, instead of starting a second one."""
        monkeypatch.delenv('HEVOLVE_VLM_CAPTION_PORT', raising=False)
        cfg, result, propagate = self._start(tmp_config_dir, alive={8080})
    ...
    def test_a_pending_llama_upgrade_waits_for_the_caption_server(self, tmp_config_dir, monkeypatch):
        ...
        with patch.object(cfg, 'check_server_type',
            ...
            cfg._do_start_server(model_preset=None)
